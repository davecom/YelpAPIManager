...
+ (NSArray *)parseYelpSearchResponse:(id)response {
    ...
    for (NSDictionary *businessItem in businessArray) {
        YelpBusinessObject *object = [self parseToBusinessObject:businessItem];
...
+ (YelpBusinessObject *)parseYelpBusinessResponse:(id)response {
    return [self parseToBusinessObject:response];
}


+ (YelpBusinessObject *)parseToBusinessObject:(NSDictionary *)businessItem {
    ...
    //Reviews
    NSArray *reviews = businessItem[@"reviews"];
    
    if ([reviews count] > 0) {
        NSMutableArray *parsedReviews = [NSMutableArray new];
        for (NSDictionary *reviewItem in reviews) {
            YelpReviewObject *reviewObject = [YelpReviewObject new];
            reviewObject.excerpt = reviewItem[@"excerpt"];
            reviewObject.reviewId = reviewItem[@"id"];
            reviewObject.rating = [reviewItem[@"rating"] floatValue];
            reviewObject.ratingImageLargeURL = [NSURL URLWithString:reviewItem[@"rating_image_large_url"]];
            reviewObject.ratingImageSmallURL = [NSURL URLWithString:reviewItem[@"rating_image_small_url"]];
            reviewObject.ratingImageURL = [NSURL URLWithString:reviewItem[@"rating_image_url"]];
            reviewObject.timeCreated = [reviewItem[@"time_created"] integerValue];
            
            NSDictionary *reviewUser = reviewItem[@"user"];
            if (reviewUser) {
                YelpReviewUserObject *userObject = [YelpReviewUserObject new];
                userObject.userId = reviewUser[@"id"];
                userObject.username = reviewUser[@"name"];
                userObject.userImageURL = [NSURL URLWithString:reviewUser[@"image_url"]];
                reviewObject.reviewUser = userObject;
            }
            [parsedReviews addObject:reviewObject];
        }
        object.reviews = [parsedReviews copy];
    }
